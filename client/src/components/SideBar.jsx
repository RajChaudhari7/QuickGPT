...
const SideBar = ({ isMenuOpen, setIsMenuOpen }) => {
    ...
    return (
        ...
        ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
            ...
            <div className='flex-1 overflow-y-scroll mt-3 text-sm space-y-3'>
                {
                    chats.filter((chat) => chat.messages[0] ? chat.messages[0]?.content.toLowerCase().includes(search.toLowerCase()) :
                        chat.name.toLowerCase().includes(search.toLowerCase())).map((chat) => (
                            <div key={chat._id} onClick={() => { navigate('/'); setSelectedChat(chat); setIsMenuOpen(false) }}
            ...
            {/* Community Images */}
            <div className='flex items-center gap-2 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer
            hover:scale-103 transition-all ' onClick={() => { navigate('/community'); setIsMenuOpen(false) }}>
            ...
            <div className='flex items-center gap-2 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer
            hover:scale-103 transition-all ' onClick={() => { navigate('/credits'); setIsMenuOpen(false) }}>
            ...
            group '>
                ...
                {user && <img src={assets.logout_icon} className='h-5 cursor-pointer  not-dark:invert ' />}
            </div>
